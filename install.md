...
To quickly download all necessary environments, compilers, and framework archives, run the provided script from the `validation_experiment` directory:

```bash
cd validation_experiment
...
```bash
$MAMBA_ROOT_PREFIX/envs/edgesimpy_env/bin/python3 src/simulators/edgesimpy/dipdce_edgesimpy_native.py
```
