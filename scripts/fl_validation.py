...
def calibrate_simulated(duration: float, base_rate: float, size: int, slaves_list: list, bandwidth: str, loss: float, jitter_us: float, target_rtts: list, scale_rate: bool = False) -> float:
    ...
    for _ in range(8):  # 8 iterations is sufficient for ~0.1ms precision
        ...
        for n in slaves_list:
            rate_to_use = base_rate * n if scale_rate else base_rate
            res = run_simulated(duration, rate_to_use, size, n, bandwidth, loss, jitter_us, mid)
...
def main() -> None:
    ...
    for n in fl_nodes_phys:
        avg_res = run_physical_multiple_trials(24.0, 2.5 * n, 112, "rr", n, num_trials, f"fl_phys_nodes_{n}")
        fl_phys_avg.append(avg_res)
        
    phys_rtts_fl = [res["avg_rtt"] for res in fl_phys_avg]
    
    # Auto-calibrate link latency for FL scenario
    fl_latency_ms = calibrate_simulated(24.0, 2.5, 112, fl_nodes_phys, "760Mbps", 0.0, 0.0, phys_rtts_fl, scale_rate=True)
    
    # Run simulation sweep using calibrated latency
    fl_sim = []
    for n in fl_nodes_sim:
        print(f"Running FL Simulation: Nodes = {n}...")
        s_res = run_simulated(24.0, 2.5 * n, 112, n, "760Mbps", 0.0, 0.0, fl_latency_ms, f"results/fl_sim_nodes_{n}.csv")
        fl_sim.append(s_res)
    ...
    for n in sc_nodes_phys:
        avg_res = run_physical_multiple_trials(12.0, 5.0 * n, 137, "rr", n, num_trials, f"sc_phys_nodes_{n}")
    ...
    sc_latency_ms = calibrate_simulated(12.0, 5.0, 137, sc_nodes_phys, "760Mbps", 0.0, 0.0, phys_rtts_sc, scale_rate=True)
    
    # Run simulation sweep using calibrated latency
    sc_sim = []
    for n in sc_nodes_sim:
        print(f"Running Smart City Simulation: Nodes = {n}...")
        s_res = run_simulated(12.0, 5.0 * n, 137, n, "760Mbps", 0.0, 0.0, sc_latency_ms, f"results/sc_sim_nodes_{n}.csv")
        sc_sim.append(s_res)
    ...
    # End of scalability experiments
    
    # Dump metrics to a new JSON file
    summary_data = {
        "fl_scalability": {
    ...
    with open("results/fl_results.json", "w") as f:
        json.dump(summary_data, f, indent=4)
        
    print("Metrics logged successfully to results/fl_results.json")
